...
export class ShortcutActionService {
   ...
   * @param {Array<any>} arrayParentPath - Path of the element's nearest array parent
   */
  ...
  /**
   * @param {Array<any>} originalPath - Path of the element that is copied
   * @param {Array<any>} arrayParentPath - Path of the element's root array parent
   */
  ...
  addRowsAction(path: Array<any>, values: Array<string>) {
    let _path = this.pathUtilService.getNearestOrRootArrayParentInPath(path, false);
    this.addRowsForValues(path, _path, values);
  }
   ...
   * @param {Array<any>} arrayParentPath - Path of the element's nearest array parent
   */
  private addRowsForValues(originalPath: Array<any>, arrayParentPath: Array<any>, values: Array<string>) {
    let elemIndex = this.pathUtilService.getElementIndexInReversePath(originalPath);
    let valuesList = this.jsonStoreService.getIn(arrayParentPath) || List();
    for (let i = 1; i < values.length; i++) {
      let newValue = valuesList.get(elemIndex);
      newValue = newValue.set(originalPath[originalPath.length - 1], values[i]);
      valuesList = valuesList.insert(elemIndex + i, newValue);
    }
    this.jsonStoreService.setIn(arrayParentPath, valuesList);
    let pathString = `${arrayParentPath.join('.')}.${elemIndex + values.length - 1}.${originalPath[originalPath.length - 1]}`;
    setTimeout(() => {
      this.updateTabIndexesAndFocusElementInPath(pathString);
    });
  }
}
